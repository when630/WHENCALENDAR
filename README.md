...
> **상태: v0.1.0 준비 중.** 기능은 다 돌아가고 설치 파일도 만들어집니다. 아직 공개 릴리스를 올리지 않았습니다.
...
**나중에** — macOS, 타임존 전환 UI, 클립보드·음성에서 일정 낚아채기

## 설치

[Releases](../../releases)에서 `WHENCALENDAR-<버전>-win-x64.exe`를 받아 실행하면 끝입니다.
데이터베이스도, 계정도, 다른 프로그램도 필요 없습니다.

### 처음 실행할 때 경고가 뜹니다

코드 서명 인증서를 쓰지 않아서 Windows가 경고를 냅니다. 서명에는 연 수십만 원이 들고,
이 앱은 그 비용을 쓰지 않습니다. 대신 무엇을 하는 앱인지는 이 저장소의 코드가 전부입니다.
...
새 버전을 내려받아 두고 **앱을 종료할 때** 설치합니다. 쓰는 도중에 스스로 재시작하지 않습니다 —
상주 앱이 그 순간 사라지면, 놓친 일정이 이 앱이 막으려던 바로 그 일입니다.

트레이 메뉴에서 언제든 직접 확인할 수 있습니다.

### 내 데이터

`%APPDATA%\whencalendar`의 `store.sqlite` 하나에 있습니다. 설정 탭이 그 경로를 열어 주고,
...
종료는 트레이 아이콘 우클릭 → 종료. 단축키가 다른 프로그램과 겹쳐 등록되지 못하면
트레이 메뉴에 그 사실이 적힙니다 — 눌러도 안 되는 이유를 알 방법이 그것뿐입니다.

```bash
npm test         # 163개. 전부 순수 모듈이라 화면 없이 돈다
