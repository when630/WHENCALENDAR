...
| 필요한 것 | 방법 | 상태 |
|---|---|---|
| 전체화면 앱 위 | `setAlwaysOnTop(true, 'screen-saver')` | **미검증** — [오픈이슈 #6](#오픈이슈) |
...
조사 결과 `chrono-node`에 **한국어 로케일이 없다**(2026-09-17 context7·웹 확인. `en`·`ja`·`fr` 등만 있음). 다만 커스텀 `parsers`/`refiners` 확장 API가 잘 갖춰져 있어 **chrono 위에 한국어 어휘만 얹는다** — 시각 결합·범위 병합·상대일 계산 파이프라인은 chrono 것을 그대로 쓴다.
...
| 2 | **전역 단축키 `Ctrl+Alt+C`.** 형제 앱(`Space`·`M`·`N`)과는 안 겹치지만 다른 프로그램과 겹칠 수 있다. PoC에서 `Ctrl+Alt+Q`가 예기치 않게 눌린 적이 있어 실사용 확인이 필요하다 | 열림 |
| 3 | **`.ics` 파서 라이브러리 선정.** 후보는 `ical.js`·`node-ical`. RRULE 펼치기를 파서가 해 주는지, 별도 `rrule` 라이브러리가 필요한지 포함해 **착수 전 context7으로 현재 문서를 확인한다** | 열림 |
...
| 6 | **오버레이가 전체화면 앱 위에 뜨는가 (OVL-02).** `screen-saver` 레벨로 설정했으나 Windows 전체화면 앱(IDE·게임·영상) 위에서 실측하지 않았다. **이게 안 되면 컨셉의 절반이 무너진다 — Phase 1의 첫 항목** | 열림 |
